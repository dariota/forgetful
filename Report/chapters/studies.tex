...
\begin{table}
	...
	\begin{tabularx}{\linewidth}{>{\hsize=0.6\hsize}X >{\hsize=1.4\hsize}X}
		...
		\functionname{external} & Performs no allocation, instead taking a parameter of a pointer to sufficiently large buffer to copy data to, which it doesn't \free{}. Acts as a control as no allocation is performed and so it should always be the fastest \\
		\bottomrule
	\end{tabularx}
