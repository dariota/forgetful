...
def process_csv(csv):
    ...
    for row in csvs[olevel]:
        ...
        results[memtype][size].append(ops / time)
...
def score_results(count, stats):
    ...
    def score_deviation(target, results):
        results = list(filter((lambda x: x[1] != "EXTERNAL"), results))
        target["deviation"] = {}
        target["deviation"]["max"] = (1 - (results[-1][0]/results[0][0])) * 100
        target["deviation"]["min"] = (1 - (results[1][0]/results[0][0])) * 100
        deviations = []
...
for olevel in computed:
    for i_count in computed[olevel]:
        for category in computed[olevel][i_count]:
            results = sorted(computed[olevel][i_count][category], reverse=(category != "mins"))
            computed[olevel][i_count][category] = list(map((lambda x: [x[0], x[1]]), results))
